...
age_selection=st.slider('Age:',
min_value=min(ages),max_value=max(ages),value=(min(ages),max(ages)))
...
#chart
bar_chart=px.bar(df_group,x='Rating',y='Votes',text='Votes',color_discrete_sequence=['#F63366']*len(df_group),
                   template= 'plotly_white')
st.plotly_chart(bar_chart)

col1,col2=st.columns(2)
image=Image.open('Images/survey.jpg')
col1.image(image, caption='Designed by Slidego / Freepik',
use_column_width=True)
